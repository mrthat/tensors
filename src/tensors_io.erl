...
get_range(Riak, File, Start, Length) ->
    ...
    {ok, RootObj} = tensors_tree:get(Riak, Root),
    Blocks = tensors_tree:get_range(Riak, RootObj, BlockSize, 0,
                                  Start, Start+Length),
...
%% @spec truncate(Riak :: riak(), File :: tensors_file(), Start :: int()) ->
%%       {ok, NewFile}
...
truncate(Riak, File, 0) ->
    tensors_file:update_root(Riak, File, undefined);
truncate(Riak, File, Start) ->
    RootName = tensors_file:get_property(File, root),
    BlockSize = tensors_file:get_property(File, block_size),
    Order = tensors_file:get_property(File, tree_order),
    {ok, Root} = tensors_tree:get(Riak, RootName),
    {ok, {NewRootName,_}} =
        tensors_tree:truncate(Riak, File, Start, Root, Order, 0, BlockSize),
...
no_tree_put_range(Riak, File, Start, Data) ->
    ...
    case (Start rem BlockSize) of
        ...
        _BlockOffset ->
            ...
            {ok, Block} = tensors_tree:block_at(Riak, File, Start-_BlockOffset),
    ...
    case (Start rem BlockSize) of
        0 ->
            {ok, Written} =
                write_blocks(Riak, File, undefined, Start, Data, BlockSize, []),
            {ok, NewFile} =
                tensors_tree:update(Riak, File, BlockAlignedStart, Written),
            {ok, NewFile1} =
                tensors_file:update_checksum(Riak, NewFile,
...
write_blocks(Riak, File, undefined, Start, Data, BlockSize, Written)
  when is_list(Written), byte_size(Data) < BlockSize ->
    ...
    case tensors_tree:block_at(Riak, File, Start) of
        {ok, undefined} ->
            {ok, NewBlock} = tensors_block:create(Riak, Data),
            {ok, lists:reverse([{tensors_block:name(NewBlock),
                                 byte_size(Data)}|
                                Written])};
        {error, notfound} ->
            {ok, NewBlock} = tensors_block:create(Riak, Data),
            {ok, lists:reverse([{tensors_block:name(NewBlock),
                                ...
                                Written])};
        {ok, Block} ->
            BlockData = case tensors_block:data(Block) of
            ...
            {ok, NewBlock} = tensors_block:create(Riak, BlockData),
            {ok, lists:reverse([{tensors_block:name(NewBlock),
                                 byte_size(BlockData)}|
                                Written])}
    end;
...
write_blocks(Riak, File, undefined, Start, Data, BlockSize, Written)
  when is_list(Written) ->
    ...
    {ok, Block} = tensors_block:create(Riak, Slice),
    write_blocks(Riak, File, undefined, Start+BlockSize, Tail, BlockSize,
                 [{tensors_block:name(Block),BlockSize}|Written]);
...
write_blocks(Riak, _File, PartialStartBlock, Start, Data, BlockSize, Written)
  when is_list(Written), byte_size(Data) < BlockSize,
    ...
    <<Head:PartialStart/binary, _:DataSize/binary, Tail/binary>> =
        tensors_block:data(PartialStartBlock),
    BlockData = <<Head/binary, Data/binary, Tail/binary>>,
    {ok, Block} = tensors_block:create(Riak, BlockData),
    {ok, lists:reverse([{tensors_block:name(Block),
                         byte_size(BlockData)}|
                        Written])};
%% sub block write on a partial write
write_blocks(Riak, _File, PartialStartBlock, Start, Data, BlockSize, Written)
  when is_list(Written), byte_size(Data) < BlockSize ->
  PartialStartData = tensors_block:data(PartialStartBlock),
  DataSize = byte_size(Data),
  PartialStart = Start rem BlockSize,
  ...
  {ok, Block} = tensors_block:create(Riak, BlockData),
  {ok, lists:reverse([{tensors_block:name(Block),byte_size(BlockData)}|Written])};
...
write_blocks(Riak, File, PartialStartBlock, Start, Data, BlockSize, Written)
  when Start > BlockSize ->
    ...
    <<Head:HeadSize/binary,_/binary>> = tensors_block:data(PartialStartBlock),
    ...
    write_blocks(Riak, File, undefined, Start+byte_size(BlockData), Tail,
                 BlockSize,
                 [{tensors_block:name(Block),byte_size(BlockData)}|Written]);
write_blocks(Riak, File, PartialStartBlock, Start, Data, BlockSize, Written)
  when is_list(Written) ->
    ...
    <<Head:Start/binary, _/binary>> = tensors_block:data(PartialStartBlock),
...
retrieve_blocks(Riak, [{Name,_}], _, Length, Acc) ->
    {ok, Block} = tensors_tree:get(Riak, Name),
    PreData = tensors_block:data(Block),
...
retrieve_blocks(Riak, [{Name,_}|Children], ChopHead, Length, Acc) ->
    {ok, Block} = tensors_tree:get(Riak, Name),
    <<_:ChopHead/binary, Data/binary>> = tensors_block:data(Block),
