-module(tensors_tree_utils_tests).
...
left_list_larger_subtree_test() ->
    ...
    ?assertEqual({{[{a,100}, {b,50}], [{c,200}]},
                 ...
                 tensors_tree_utils:longest_divisable_subtree(A, B)).
...
right_list_larger_subtree_test() ->
    ...
    ?assertEqual({{[{d,50}, {e,50}, {f,50}, {g,50}, {h,50}], []},
                  {[{a,100}, {b,50}, {c,200}], []}},
                 tensors_tree_utils:longest_divisable_subtree(B, A)).
...
split_at_length_test() ->
    ...
    ?assertEqual({[{a,100},{b,50}],
                  [{c,200}]},
                 tensors_tree_utils:split_at_length(A, 200)).

split_at_length_left_bias_test() ->
    A = [{a,100},{b,50},{c,200}],
    ?assertEqual({A,[]},
                 tensors_tree_utils:split_at_length_left_bias(A, 200)).

simple_five_way_split_test() ->
    Nodes = [{a,100},{b,100},{c,100},{d,100},{e,100}],
    Blocks = [{f,50},{g,50},{h,50},{i,50}],
    {NodeSplit,BlockSplit} = tensors_tree_utils:five_way_split(0, Nodes, 150, Blocks),
    ?assertEqual(#split{
                    head=[{a,100}],
                    midhead=[{b,100}],
                    middle=[{c,100}],
...
no_head_five_way_split_test() ->
    Nodes = [{a,100},{b,100},{c,100},{d,100},{e,100}],
    Blocks = [{f,50},{g,50},{h,50},{i,50}],
    {NodeSplit,BlockSplit} = tensors_tree_utils:five_way_split(0, Nodes, 50, Blocks),
    ?assertEqual(#split{
                    head=[],
                    midhead=[{a,100}],
                    middle=[{b,100}],
...
equal_start_five_way_split_test() ->
    Nodes = [{a,100},{b,100},{c,100},{d,100},{e,100}],
    Blocks = [{f,50},{g,50},{h,50},{i,50}],
    {NodeSplit,BlockSplit} = tensors_tree_utils:five_way_split(0, Nodes, 0, Blocks),
    ?assertEqual(#split{
                    head=[],
                    midhead=[],
                    middle=[{a,100},{b,100}],
...
appending_five_way_split_test() ->
    ...
    _A = {NodeSplit,_BlockSplit} = tensors_tree_utils:five_way_split(0, Nodes, 12, Blocks),
    ?assertEqual(#split{
                    head=[{a,6},{b,6}],
...
first_block_five_way_split_test() ->
    Nodes = [{a,1},{b,1},{c,1}],
    Blocks = [{a,1}],
    {NodeSplit, BlockSplit} = tensors_tree_utils:five_way_split(9, Nodes, 9, Blocks),
...
subtree_split_test() ->
    ...
    ?assertEqual({{[{a,50},{b,50}],[{c,50}]},
                  {[{d,150}],[{e,100}]}},
                 tensors_tree_utils:shortest_subtree_split(ListA, ListB, 50, 0)).
