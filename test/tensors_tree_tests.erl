...
create_simple_tree() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{tree_order,4},{block_size,5}], dict:new()),
      {ok, _Written, File2} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklm">>),
      ...
      Root = tensors_file:get_property(File2, root),
      ?assertEqual(RootHash, Root),
      {ok, RootNode} = tensors_tree:get(Riak, Root),
...
create_and_overwrite_middle_tree() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{tree_order,4},{block_size,5}], dict:new()),
      {ok, _Written1, File2} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklm">>),
      ...
      {ok, _Written2, File3} = tensors_io:put_range(Riak, File2, 1, <<"zyxwvutsrq">>),
      ...
      Root = tensors_file:get_property(File3, root),
...
create_multilevel_tree() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{tree_order,5},{block_size,1}], dict:new()),
      {ok, _Written1, File2} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklm">>),
      ...
      ?assertEqual(Root1, tensors_file:get_property(File2, root)),
      {ok, RootNode} = tensors_tree:get(Riak, Root1),
      ?assertEqual(RootChildren, RootNode#n.children),
      {ok, Node1Node} = tensors_tree:get(Riak, Node1),
      {ok, Node2Node} = tensors_tree:get(Riak, Node2),
      {ok, Node3Node} = tensors_tree:get(Riak, Node3),
...
create_and_overwrite_multilevel_tree() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{tree_order,5},{block_size,1}], dict:new()),
      {ok, _Written1, File2} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklm">>),
      % ok = file:write_file("tree1.dot", tensors_tree:visualize_tree(Riak, tensors_file:get_property(File2, root))),
      {ok, _Written2, File3} = tensors_io:put_range(Riak, File2, 1, <<"zyxwvutsrq">>),
      ...
      RootChildren = [{Node1,5}, {Node2,5}, {Node3,3}],
      Root1 = skerl:hexhash(512, term_to_binary(RootChildren)),
      ok = file:write_file("tree2.dot", tensors_tree:visualize_tree(Riak, tensors_file:get_property(File3, root))),
      ?assertEqual(Root1, tensors_file:get_property(File3, root)),
            {ok, RootNode} = tensors_tree:get(Riak, Root1),
      ?assertEqual(RootChildren, RootNode#n.children),
      {ok, Node1Node} = tensors_tree:get(Riak, Node1),
      {ok, Node2Node} = tensors_tree:get(Riak, Node2),
...
create_and_append() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{tree_order,3},{block_size,2}], dict:new()),
      {ok, _Written1, File2} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklm">>),
      {ok, _Written2, File3} = tensors_io:put_range(Riak, File2, 13, <<"nopqrstuvwxyz">>),
      _Blocks = [ {skerl:hexhash(512, X), 2} || <<X:2/binary>> <= <<"abcdefghijklmnopqrstuvwxyz">> ],
      ok = file:write_file("tree3.dot", tensors_tree:visualize_tree(Riak, tensors_file:get_property(File3, root))),
      ?assertEqual(<<"abcdefghijklmnopqrstuvwxyz">>, iolist_to_binary(tensors_io:get_range(Riak, File3, 0, 26)))
    end).
...
append_beyond_pointer() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{tree_order,3},{block_size,2}], dict:new()),
      {ok, _Written1, File2} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklm">>),
      {ok, _Written2, File3} = tensors_io:put_range(Riak, File2, 15, <<"nopqrstuvwxyz">>),
      ?assertEqual(<<"abcdefghijklmnopqrstuvwxyz">>, iolist_to_binary(tensors_io:get_range(Riak, File3, 0, 26)))
    end).

block_at() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{tree_order,3},{block_size,3}], dict:new()),
      {ok, _Written1, File1} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklmno">>),
      {ok, Block} = tensors_tree:block_at(Riak, File1, 9),
