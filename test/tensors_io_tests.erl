...
simple_put_range() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{block_size,5}], dict:new()),
      {ok, Written, _} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklm">>),
      ...
      {ok, Block1} = Riak:get(<<"tensors_node">>, Hash1, 2),
      {ok, Block2} = Riak:get(<<"tensors_node">>, Hash2, 2),
      {ok, Block3} = Riak:get(<<"tensors_node">>, Hash3, 2),
      ?assertEqual(<<"abcde">>, tensors_block:data(Block1)),
      ?assertEqual(<<"fghij">>, tensors_block:data(Block2)),
      ?assertEqual(<<"klm">>, tensors_block:data(Block3)),
      ?assertEqual([{Hash1,5},{Hash2,5},{Hash3,3}], Written)
    end).

simple_get_range() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{block_size,2},{tree_order,3}], dict:new()),
      {ok, _Written, File1} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklm">>),
      Blocks = tensors_io:get_range(Riak, File1, 3, 5),
      ok = file:write_file("tree4.dot", tensors_tree:visualize_tree(Riak, tensors_file:get_property(File1, root))),
...
read_beyond_eof() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{block_size,2},{tree_order,3}], dict:new()),
      {ok, _Written, File1} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklm">>),
      Blocks = tensors_io:get_range(Riak, File1, 14, 5),
      ?assertEqual([], Blocks)
    end).
...
multilevel_get_range() ->
  test_helper:riak_test(fun(Riak) ->
      ...
      Blocks = tensors_io:get_range(Riak, File1, 4, 9),
      ok = file:write_file("tree5.dot", tensors_tree:visualize_tree(Riak, tensors_file:get_property(File1, root))),
    ...
    test_helper:riak_test(
      fun(Riak) ->
              ...
              Read = iolist_to_binary(
                       tensors_io:get_range(Riak, File, 10, 14)),
...
partial_start_block_get_range() ->
    test_helper:riak_test(
      fun(Riak) ->
              ...
              Read = iolist_to_binary(
                       tensors_io:get_range(Riak, File, 5, 15)),
    ...
    test_helper:riak_test(
      fun(Riak) ->
              {ok, Data, File} = make_standard_range_file(
               ...
               Read = iolist_to_binary(
                        tensors_io:get_range(Riak, File, 11, 8)),
...
eof_get_range() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{block_size,3},{tree_order,3}], dict:new()),
      {ok, _Written, File1} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklmnopqrstuvwxyz">>),
      ok = file:write_file("tree6.dot", tensors_tree:visualize_tree(Riak, tensors_file:get_property(File1, root))),
      Blocks = tensors_io:get_range(Riak, File1, 20, 20),
...
truncate() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"file1">>, [{block_size,3},{tree_order,3}], dict:new()),
      {ok, _Written, File1} = tensors_io:put_range(Riak, File, 0, <<"abcdefghijklmnopqrstuvwxyz">>),
      {ok, File2} = tensors_io:truncate(Riak, File1, 7),
      ok = file:write_file("tree7.dot", tensors_tree:visualize_tree(Riak, tensors_file:get_property(File2, root))),
      Blocks = tensors_io:get_range(Riak, File2, 0, 7),
      ?assertEqual(<<"abcdefg">>, iolist_to_binary(Blocks))
    end).

sub_block_partial_write() ->
  test_helper:riak_test(fun(Riak) ->
      {ok, File} = tensors_file:create(Riak, <<"basho">>, [{block_size,1000}], dict:new()),
      S = tensors_put_stream:start_link(Riak, File, 0, 1000),
      B1 = crypto:rand_bytes(33),
      tensors_put_stream:send(S, B1),
      tensors_put_stream:close(S),
      timer:sleep(100),
      {ok, File2} = tensors_file:get(Riak, <<"basho">>),
      S1 = tensors_put_stream:start_link(Riak, File2, 33, 1000),
      B2 = crypto:rand_bytes(33),
      tensors_put_stream:send(S1, B2),
      tensors_put_stream:close(S1),
      timer:sleep(100),
      {ok, File3} = tensors_file:get(Riak, <<"basho">>),
      Blocks = tensors_io:get_range(Riak, File3, 0, 66),
...
overwrite_nonfull_block() ->
    test_helper:riak_test(fun(Riak) ->
        {ok, F0} = tensors_file:create(Riak,
                                     <<"basho1">>,
                                     [{block_size, 4}],
                                     dict:new()),
        S0 = tensors_put_stream:start_link(Riak, F0, 0, 1000),
        tensors_put_stream:send(S0, <<"a">>),
        tensors_put_stream:close(S0),
        timer:sleep(100),

        {ok, F1} = tensors_file:get(Riak, <<"basho1">>),
        S1 = tensors_put_stream:start_link(Riak, F1, 0, 1000),
        tensors_put_stream:send(S1, <<"aa">>),
        tensors_put_stream:close(S1),
        timer:sleep(100),

        {ok, F2} = tensors_file:get(Riak, <<"basho1">>),
        Blocks = tensors_io:get_range(Riak, F2, 0, 4),
        ?debugFmt("blocks ~p~n", [Blocks]),
